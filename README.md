...
- Various methods to work with bit counting are defined under a built-in class called `bit`, which is documented [here](./doc/bit.md)
...
- Various methods to work with formatting output and printing it are defined under a built-in class called `fmt`, which is documented [here](./doc/fmt.md)
...
- Various methods and fields to work with the state of this interpreter are defined under a built-in class called `lox`, where the following methods and fields are defined:
    ...
    - `lox.globalDNS()`, which returns a string representing the global DNS IP if one was set using the `--dns` flag, or an empty string otherwise
    ...
    - `lox.usingGlobalDNS()`, which returns `true` if a global DNS IP was specified using the `--dns` flag and `false` otherwise
...
- A few other native functions are defined:
    ...
    - `repeatFuncNoErr(integer, callback)`, which takes in an integer and callback function and repeatedly invokes the callback function for the specified integer number of times, without stopping even if a callback function throws an error
        - If the specified integer argument is negative, it is the same as specifying `0` as the integer argument
